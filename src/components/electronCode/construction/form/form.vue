...
<template>
  <div>
    <div class="content-right min-gutter">
      <h3>{{action === 'add' ? '添加' : '编辑'}}货品电子监管码</h3>
      <div>
        <el-form ref="goodSForm" :model="form" :rules="rules" label-width="120px"
          ...
          <el-form-item label="货品" prop="goodsId">
            <el-select placeholder="请输入名称搜索货品" v-model="form.goodsId" filterable remote
                       :remote-method="queryManageGoods" :clearable="true"
          ...
          <el-form-item label="电子监管码" prop="code">
            <oms-input type="text" v-model="form.code" placeholder="电子监管码"></oms-input>
          </el-form-item>
          <el-form-item >
...
<script type="text/jsx">
  ...
  export default {
    mixins: [ScrollMixin, methodsMixin],
    data: function () {
      return {
        form: {
          goodsId: '',
        ...
        rules: {
          ...
          code: [
            {required: true, message: '请输入电子监管码', trigger: 'blur'}
          ]
        },
        goodsList: [], // oms货品列表
        doing: false,

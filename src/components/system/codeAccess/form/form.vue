...
<template>
  <div>
    ...
    <el-form ref="accountform" :model="form" label-width="100px" :rules="rules"
      ...
      <el-form-item label="授权单位" prop="objectOrgId">
        <org-select :list="allOrgList" :remoteMethod="queryUpAllFactory"
                    placeholder="请输入名称搜索授权单位" v-model="form.objectOrgId"></org-select>
      </el-form-item>
      <el-form-item label="被授权单位" prop="subjectOrgId">
        <org-select :list="downOrgList" :remoteMethod="queryDownAllFactory"
                    placeholder="请输入名称搜索被授权单位" v-model="form.subjectOrgId"></org-select>
      </el-form-item>
  ...
  export default {
    ...
    methods: {
      queryManageGoodsNew(query) { // 查询平台货品
        this.manageGoods = [];
        if (!this.form.objectOrgId) {
          return this.$notify.info({message: '请选择授权单位'});
        }
        let params = {
          orgId: this.form.objectOrgId,
          deleteFlag: false,
          keyWord: query
        };
